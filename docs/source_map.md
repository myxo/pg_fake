...
[`api.rs`](../crates/pg_fake/src/api.rs) lists the public types and preserves
their `pg_fake::api` import paths. Implementation modules are private.

| Module | Responsibility |
| --- | --- |
| [`api/database.rs`](../crates/pg_fake/src/api/database.rs) | Database construction, shared engine state, mock time, and random seed configuration. |
| [`api/results.rs`](../crates/pg_fake/src/api/results.rs) | Result rows, column metadata, and affected-row counts. |
| [`api/catalog_inspection.rs`](../crates/pg_fake/src/api/catalog_inspection.rs) | Stable catalog descriptions for diagnostics and differential comparisons. |
| [`api/session/mod.rs`](../crates/pg_fake/src/api/session/mod.rs) | Session state, statement batches, snapshots, and execution order. |
| [`api/session/prepared.rs`](../crates/pg_fake/src/api/session/prepared.rs) | Parameter inference and binding, prepared handles, and cached read-lock requirements. |
| [`api/session/catalog_dependencies.rs`](../crates/pg_fake/src/api/session/catalog_dependencies.rs) | Discover tables, views, sequences, and constraints referenced by SQL; validate saved dependencies against the visible catalog. |
| [`api/session/transactions.rs`](../crates/pg_fake/src/api/session/transactions.rs) | SQL transaction control, the transaction guard, commit/abort cleanup, version reclamation, and session teardown. |
| [`api/session/settings.rs`](../crates/pg_fake/src/api/session/settings.rs) | Session settings, parsing their values, and saving/restoring settings around transactions. |
| [`api/session/constraint_timing.rs`](../crates/pg_fake/src/api/session/constraint_timing.rs) | `SET CONSTRAINTS` and switching deferred checks to immediate checks. |
| [`api/session/do_block.rs`](../crates/pg_fake/src/api/session/do_block.rs) | Anonymous PL/pgSQL blocks: local variables, control flow, and nested SQL sharing the block's deadline and statement timestamp. |
...
[`api/session/locking`](../crates/pg_fake/src/api/session/locking/mod.rs) coordinates
...
- [`relations.rs`](../crates/pg_fake/src/api/session/locking/relations.rs) collects
  relation lock requirements for queries, mutations, and explicit `LOCK TABLE`.
- [`ddl.rs`](../crates/pg_fake/src/api/session/locking/ddl.rs) handles schema
  changes and their dependent objects.
- [`foreign_keys.rs`](../crates/pg_fake/src/api/session/locking/foreign_keys.rs)
...
[`api/session/tests.rs`](../crates/pg_fake/src/api/session/tests.rs). Public API
