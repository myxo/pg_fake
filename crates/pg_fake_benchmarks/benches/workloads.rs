...
fn serial_identity_benchmark(
    ...
    for (name, connection) in connections.iter_mut() {
        group.bench_function(*name, |benchmark| {
            benchmark.iter(|| {
                connection.fetch(
                    runtime,
                    "INSERT INTO serial_identity_insert (token) VALUES ('12345678-1234-1234-1234-123456789abc') RETURNING id",
                );
            });
        });
    }
    group.finish();
    for (_, connection) in connections.iter_mut() {
        connection.execute(runtime, "DROP TABLE serial_identity_insert");
    }
}

fn uuid_temporal_benchmark(
    criterion: &mut Criterion,
    runtime: &Runtime,
    connections: &mut [NamedBenchmarkConnection<'_>],
) {
    let values = (1..=100)
        .map(|id| {
            format!("('00000000-0000-0000-0000-{id:012}', '2024-01-02 03:04:05+00', '1 day')")
        })
        .collect::<Vec<_>>()
        .join(",");
    for (_, connection) in connections.iter_mut() {
        connection.execute(
            runtime,
            "CREATE TABLE uuid_temporal_select (id UUID PRIMARY KEY, observed_at TIMESTAMPTZ, span INTERVAL)",
        );
        connection.execute(
            runtime,
            &format!("INSERT INTO uuid_temporal_select VALUES {values}"),
        );
    }
    let mut group =
    ...
    for (name, connection) in connections.iter_mut() {
        group.bench_function(*name, |benchmark| {
            benchmark.iter(|| {
                connection.fetch(
                    runtime,
                    "SELECT id, observed_at + span FROM uuid_temporal_select WHERE id = '00000000-0000-0000-0000-000000000050'",
                );
            });
        });
    }
