...
pub const FEATURES: &[Feature] = &[
    ...
    Feature {
        name: "phase 2 transaction integration",
        cases: &[Case {
            id: "joined_generated_temporal_rows",
            source: "focused Phase 2 integration scenario",
            setup: &[
                "CREATE TABLE phase2_parent (id SERIAL PRIMARY KEY, name TEXT)",
                "CREATE TABLE phase2_child (id BIGINT GENERATED BY DEFAULT AS IDENTITY, parent_id INTEGER REFERENCES phase2_parent(id), token UUID, observed_at TIMESTAMPTZ, span INTERVAL)",
                "BEGIN",
                "INSERT INTO phase2_parent (name) VALUES ('first'), ('second') RETURNING id",
                "INSERT INTO phase2_child (parent_id, token, observed_at, span) VALUES (1, '12345678-1234-1234-1234-123456789abc', '2024-01-02 03:04:05+00', '1 day'), (1, '12345678-1234-1234-1234-123456789abd', '2024-01-03 03:04:05+00', '2 days') RETURNING id",
                "SELECT parent_id, count(*) FROM phase2_child GROUP BY parent_id HAVING count(*) > 1",
                "COMMIT",
            ],
            sql: "SELECT DISTINCT parent.id, (SELECT count(*) FROM phase2_child correlated WHERE correlated.parent_id = parent.id), child.token, child.observed_at, child.span FROM phase2_parent parent INNER JOIN phase2_child child ON child.parent_id = parent.id WHERE EXISTS (SELECT 1 FROM phase2_child correlated WHERE correlated.parent_id = parent.id) ORDER BY parent.id, child.token",
            baseline: Baseline::MustPass,
        }],
    },
];
